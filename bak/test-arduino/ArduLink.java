...
public class ArduLink {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ArduLink.class);

	public static void main(String[] args) throws Exception {
		
		
		new ArduLink().writeString("aaa");
	}
	...
	public void writeString(String msg) {
		try {
			//this.link = Link.getDefaultInstance().connect("/dev/ttyACM0", "9600");//Links.getLink(new URI("ardulink://serial-jssc?port=/dev/ttyACM0"));
			link.sendCustomMessage(msg);
		} catch (IOException e) {
			logger.error("can't send msg",e);
		}
	}
